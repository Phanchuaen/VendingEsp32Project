...
#define _SSID "Vending"                                             // Your WiFi SSID
...
#define LINE_TOKEN "bDISSgHO6iFB9qIPCdPmRuGSYZpiXxVFi4G2ZhYhehK"
  ...
  char key = keypad.getKey();  // อ่านค่าจาก Keypad
    ...
    if (key == '#') {    // หากกดปุ่ม #
      ...
      if (millis() - startTime >= 5000 && !isAdminVerified) {
        ...
        resetMode = false;  // ออกจากโหมดรีเซ็ต
        delay(1000);        // หน่วงเวลาแสดงผล timeout
        lcd.clear();
    ...
    if (resetMode && isAdminVerified) {
      if (key == '*') {  // หากกดปุ่ม * ให้ออกโหมดรีเซ็ต


        // รีเซ็ตสถานะโหมดรีเซ็ตและการยืนยันผู้ดูแลระบบ
        resetMode = false;
        isAdminVerified = false;

        // แสดงผลกลับมาที่หน้าแรก
        delay(500);  // หน่วงเวลาให้เห็นข้อความ
        lcd.clear();
        lcd.setCursor(0, 0);
        lcd.print("Token Number:");
        return;  // ออกจากฟังก์ชัน loop นี้เพื่อกลับไปที่ลูปหลัก
      }

      // ส่วนของการเลือกเซอร์โว 1-9 ที่มีอยู่แล้ว
      if (key >= '1' && key <= '9') {
          ...
          while (millis() - startTime < 5000) {
            if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
              ...
              if (uidFound) {
                ...
                String lineMessage = "สินค้าข่องที่ " + String(key) + " ได้ขายไปแล้ว";
                LINE.send(lineMessage);
                Serial.println("Line notification sent: " + lineMessage);
                if (dataValue == 1) {
                  // แจ้งเตือนเมื่อสินค้าหมด
                  String outOfStockMessage = "สินค้าช่องที่ " + String(key) + " หมด";
                  LINE.send(outOfStockMessage);
                  Serial.println("Line warning sent: " + outOfStockMessage);
                } 
                if (dataValue == 2) {
                  // แจ้งเตือนเมื่อสินค้าคงเหลือน้อยกว่า 2
                  String warningMessage = "สินค้าช่องที่ " + String(key) + " ใกล้หมด";
                  LINE.send(warningMessage);
                  Serial.println("Line warning sent: " + warningMessage);
                }
